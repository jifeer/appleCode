iopctrl = function () {
  ...
  iopctrl.led = function () {
    var width = 20, height = 10, round = 2, on = true, colorOn = d3.rgb("lightgreen"), colorOff = colorOn.darker(2),
  ...
  iopctrl.slider = function () {
    var width = 50, minEventInterval = 100, enableFlick = true, transitionDuration = 500, events = true,
      moveToTouch = true, ease = "cubic-out";
    ...
    var _range, _invert, _vertical, _lastEvent, _currentValue, _currentPoint, _slide, _deltaX = 0, _deltaY = 0,
      _lastPos = {};
    var _onValueChanged;
    var _indicator;
  ...
  iopctrl.arcslider = function () {
    var radius = 100, minEventInterval = 100, enableFlick = true, transitionDuration = 500, arcFactor = 0.5,
      events = true, moveToTouch = true, ease = "cubic-out";
  ...
  iopctrl.arcaxis = function () {
    var scale = d3.scale.linear(), outerRadius = 100, innerRadius = 100, orient = "out", tickMajorSize = 6,
      tickMinorSize = 4, tickEndSize = 6, tickPadding = 3, tickArguments_ = [10], tickValues = null, tickFormat_,
      tickSubdivide = 0, normalize = true;
    ...
    function arcaxis(g) {
      g.each(function () {
        ...
        var lineEnter = tickEnter.select("line")
          ...
          , text = tick.select("text").text(function (d) {
            var num = +(tickFormat(d))
            if (num % 1 == 0) {
              return (num).toFixed()
            }
          })
  ...
  function iopctrl_axisSubdivide(scale, ticks, m) {
    subticks = [];
    if (m && ticks.length > 1) {
      var extent = iopctrl_extent(scale.domain()), subticks, i = -1, n = ticks.length, d = (ticks[1] - ticks[0]) / ++m,
        j, v;
      while (++i < n) {
        for (j = m; --j > 0;) {
