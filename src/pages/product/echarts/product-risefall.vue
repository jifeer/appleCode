                ...
                if (val.seriesIndex == 0 && (val.data.typeVal || val.data.name)) {
                  if (this.echartsData.data.length > 0 && this.echartsData.data[val.dataIndex].value <= 0) {
                    str = `<ul style="list-style: none;text-align: left;padding: 6px;">
                          <li>${val.name}</li>
                          <li>${val.data.name}：${val.data.typeVal.toFixed(2)}${val.data.name === '采购总量' ? '万亩' : val.data.name === '产量' ? '个' : '万元/个'}</li>
                          <li>${val.data.name === '产量' ? '减产' : '减少'}：${val.data.riseorfall || val.data.riseorfall === 0 ? val.data.riseorfall.toFixed(2) : "-"}${val.data.name === '采购总量' ? '万亩' : val.data.name === '产量' ? '个' : '万元/个'}</li>
                  ...
                  if (this.echartsData.data.length > 0 && this.echartsData.data[val.dataIndex].value >= 0) {
                    str = `<ul style="list-style: none;text-align: left;padding: 6px;">
                          <li>${val.name}</li>
                          <li>${val.data.name}：${val.data.typeVal.toFixed(2)}${val.data.name === '采购总量' ? '万亩' : val.data.name === '产量' ? '个' : '万元/个'}</li>
                          <li>${val.data.name === '产量' ? '增产' : '增加'}：${val.data.riseorfall || val.data.riseorfall === 0 ? val.data.riseorfall.toFixed(2) : "-"}${val.data.name === '采购总量' ? '万亩' : val.data.name === '产量' ? '个' : '万元/个'}</li>
